...
Download the [latest release binary](https://github.com/insighio/insighioNode/releases/latest). The archive is at compressed form (_tar.gz_) and can be found under the release **assets**. The file name is:
...
{{% notice warning %}}
There might be **releases with no assets** at all. These are **pre-release** versions, so we recommend to work with a version that has built assets.
{{% /notice %}}

After downloading the compressed file, extract it locally. The archive includes the required **binaries** and flash **scripts** for Windows (bat) and Linux (sh).

### esptool dependency

The flash scripts use [esptool](https://github.com/espressif/esptool) to download the binaries on the board, so before proceeding make sure **esptool** is installed.
...
**Option 2:** Another option is to directly use the executable version of esptool, i.e. `esptool.exe`.
   - Download: Go to [Official Project Releases Page](https://github.com/espressif/esptool/releases) which provides the binarier for all platforms and select the Windows version. For example for **esptool version 4.10.0** go to [this](https://github.com/espressif/esptool/releases/download/v4.10.0/esptool-v4.10.0-windows-amd64.zip) link.
...
{{% notice tip %}}
**Pro Tip**: The official documentation for **esptool** is available [in this link](https://docs.espressif.com/projects/esptool/en/latest/esp32/).
{{% /notice %}}
...
![TAT image](/images/device-firmware-flash-win.png?width=30pc)
...
After a successful installation you should see an output like this in the prompt:
![TAT image](/images/device-firmware-flash-linux.png?width=30pc)
